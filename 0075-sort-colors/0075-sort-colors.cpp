    ...
    void sortColors(vector<int>& nums) {
        ...
        while(mid<= high){// condition dont mug up just visualize by help of diagram  above  when crosses what and why?
            if(nums[mid] == 0) {
                ...
                mid++;// no changes becz its already sorted ansd 1 aree sin sorted order  just move the pointer away

            }
            else swap(nums[mid], nums[high--]);// keep the nums[mid] to the right ward in diagram  for 2 to be in sorted 
